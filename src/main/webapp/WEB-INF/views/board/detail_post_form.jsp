...
<div class="container">
  ...
  <c:if test="${board.userId.id == principal.user.id}">
    <a href="/board/update_post_form/${board.id}" class="btn btn-outline-success float-right m-2">수정</a>
    <button class="btn btn-outline-danger float-right m-2" id="btn-delete">삭제</button>
  </c:if>
  <br /><br />
  <br />
  ...
  <div class="card">
    ...
    <div class="card-footer">
      <button class="btn btn-outline-secondary float-right" id="btn-reply-save">등록</button>
    </div>
    <div></div>
  </div>
  <br />
    ...
    <c:forEach var="reply" items="${board.replies}">
      <li class="list-group-item d-flex justify-content-between">
        ...
        <c:if test="${reply.user.id eq principal.user.id}">
          <button class="btn btn-outline-danger btn-sm" id="btn-reply-delete" onclick="index.replyDelete(${reply.board.id}, ${reply.id})">삭제</button>
        </c:if>
      </li>
    </c:forEach>
  </ul>
